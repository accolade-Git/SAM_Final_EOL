#from ServerData import MyClass
import can
import time
import binascii
import sys
...
class CAN_Data(QMainWindow):
    def __init__(self):
        ...
        self.timer1.setInterval(1000)  # 1000 ms = 1 second
        self.timer1.timeout.connect(self.on_timer_timeout)
        self.elapsed_time = 0
        self.barcode = None
        self.stage5_url = "http://192.168.2.253:6101/api/stage5"
        self.totalParams = 36
        self.processed_params = 0
        self.ui.progressBar.setRange(0, 100)  # Set range from 0 to 100
        self.ui.progressBar.setValue(0)  # Initialize value
        self.totalParams = 36
        self.processed_params = 0
    ...
    "visual_inspection_timestamp": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
    "board_flashing_status": True,
    "board_flashing_timestamp": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
    "screw_fitting_status": True,
    "screw_fitting_timestamp": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
    "mechanical_fitting_status": True,
    "mechanical_fitting_timestamp": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
    "final_testing_status": True,
    "final_testing_timestamp": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',    "IMEI": None,
    "ICCID": None,
    "SystemRtc": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f'),
    "AppFWVersion": None,
    "BLFWVersion": None,
    "GPSFWVersion": None,
    "GSMFWVersion": None,
    ...
    "sticker_printing_timestamp": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.%f'),
    ...
    def check_barcode(self):
        ...
        def check_barcode_value():
            ...
            if barcode:
                ...
                # Call another function to process the device model, assuming it's defined
 
 
                # Clean the barcode text if necessary
                self.barcode = self.barcode.strip()
                print(f"Cleaned Barcode: {repr(self.barcode)}")
                ...
                print("device_status_url",self.device_status_url)
    ...
    def get_device_model(self):
        ...
        try:
            ...
            print("get_device_model", response.text)
            ...
            if response.status_code == 200:
                ...
                # Set up a QTimer to check the input field every 100 milliseconds
                timer.timeout.connect(self.check_barcode)
                timer.start(100)  # Check every 100 milliseconds
    ...
    def send_data(self):                            #Update variable here to send to server
        ...
        if self.check_previous_stages:
            #print("self.check_previous_stages", self.check_previous_stages)
            try:
                # print(f"Data to send: {self.data}")  # Log the data being sent
                # print(f"Headers: {self.headers}")  # Log the headers being sent
                ...
                # Removed the incomplete print
                if response.status_code == 200:
                    print("Data sent successfully")
                    print(response.json())  # prints the response content in JSON format
                    if self.processed_params == self.totalParams:
